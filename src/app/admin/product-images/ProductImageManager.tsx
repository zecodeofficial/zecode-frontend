...
const GALLERY_FIELD = { key: "images", label: "Product Gallery" } as const;
...
function GalleryImageCard({
  ...
  index: number | null;
  busy: boolean;
  onUpload: (file: File, name?: string, index?: number | null) => Promise<void>;
}) {
  const [rename, setRename] = useState<string>("");
  const fileInputRef = useRef<HTMLInputElement | null>(null);
  ...
  const handleFileChange = async (event: ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0];
    if (!file) return;
    await onUpload(file, rename || undefined, index);
    event.target.value = "";
  };

  const label = index === null ? "Add gallery image" : `Gallery image ${index + 1}`;
      ...
      <div className="mt-4 space-y-3">
        <div className="space-y-1">
          ...
          <input
            ...
            value={rename}
            onChange={(event) => setRename(event.target.value)}
            placeholder="e.g. product-gallery-front"
            className="w-full rounded border border-gray-300 p-2"
            disabled={busy}
          />
          <p className="text-xs text-gray-500">Provide a filename before uploading to replace or append.</p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="rounded bg-indigo-600 px-4 py-2 text-sm font-medium text-white shadow hover:bg-indigo-700 disabled:cursor-not-allowed disabled:bg-gray-300"
          >
            {busy ? "Uploading..." : index === null ? "Upload new gallery image" : "Replace image"}
          </button>
          {busy && <span className="text-sm text-gray-500">Uploading to Cloudinary and saving to gallery...</span>}
        </div>
      </div>
    </div>
  );
...
export function ProductImageManager({ productId }: { productId: string }) {
  ...
  useEffect(() => {
    ...
    fetchProduct(productId)
      .then((fetched) => {
        if (!fetched) {
          setLoadError("Product not found or inaccessible.");
        }
        setProduct(fetched);
      })
      .catch((error) => {
  ...
  const galleryImages = useMemo(() => {
    if (!product) return [] as (string | null | undefined)[];
    const raw = (product as any)?.[GALLERY_FIELD.key];
  ...
  const handleGalleryUpload = async (
    ...
    const busyKey = index === null ? `${field}-new` : `${field}-${index}`;
    ...
    setBusyField(null);
    setProduct((prev) => (prev ? { ...prev, [field]: data.urls } as Product : prev));
  };
  ...
  if (loadError || !product) {
    return <div className="p-6 text-red-600">{loadError || "Product not found"}</div>;
  }

  return (
    <div className="p-6 space-y-6 rounded border border-gray-200 bg-white shadow-sm">
      <div>
        <h1 className="text-2xl font-semibold">Manage Images for {product.name}</h1>
        <p className="text-sm text-gray-600">Uploads will replace the existing Cloudinary asset for each field.</p>
      </div>
      ...
      <div className="space-y-4">
        <div>
          <h2 className="text-xl font-semibold">{GALLERY_FIELD.label}</h2>
          <p className="text-sm text-gray-600">Replace existing gallery images or upload new ones.</p>
        </div>
        <div className="grid gap-6 md:grid-cols-2">
          {galleryImages.map((url, index) => (
            <GalleryImageCard
              key={`${GALLERY_FIELD.key}-${index}`}
              value={url}
              index={index}
              busy={busyField === `${GALLERY_FIELD.key}-${index}`}
              onUpload={(file, name, targetIndex) => handleGalleryUpload(file, name, targetIndex)}
            />
          ))}
          <GalleryImageCard
            key={`${GALLERY_FIELD.key}-new`}
            value={null}
            index={null}
            busy={busyField === `${GALLERY_FIELD.key}-new`}
            onUpload={(file, name, targetIndex) => handleGalleryUpload(file, name, targetIndex)}
          />
        </div>
      </div>
