...
const SUBCATEGORY_MAP: Record<string, string[]> = {
  'tshirts': ['T-Shirts', 'T-Shirt', 'T', 'Tshirt', 'Graphic Tee', 'Classic T-Shirt', 'Boy T-Shirt'],
  'shirts': ['Shirt', 'Shirts', 'Casual Shirt', 'Button-Up Shirt', 'Short Sleeve Shirt'],
  'jeans': ['Jeans', 'Denim', 'Slim Jeans', 'Wide Leg Jeans', 'Bottom', 'Bottoms'],
  'tops': ['Top', 'Tops', 'Casual Top', 'Tank Top', 'Girl Top'],
  'dresses': ['Dress', 'Dresses', 'Midi Dress', 'Mini Dress', 'Slip Dress', 'Girl Dress'],
  'jackets': ['Jacket', 'Jackets', 'Outerwear', 'Blazer', 'Coat', 'Casual Jacket', 'Denim Jacket', 'Varsity Jacket'],
  'jumpsuits': ['Jumpsuit', 'Jumpsuits'],
  'tracksuits': ['Tracksuit', 'Tracksuits', 'Activewear'],
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const description = SUBCATEGORY_DESCRIPTIONS.kids[subcategory] || `Shop Kids' ${displayTitle} collection at Zecode.`;
...
export default async function KidsSubcategoryPage({ params, searchParams }: PageProps) {
  ...
  if (SUBCATEGORY_MAP[subcategory]) {
    ...
    const fetchedProducts = await fetchProductsByGenderAndSubcategory("kids", cmsSubcategories);
    if (!fetchedProducts) {
        ...
        <div className="max-w-7xl mx-auto px-4 py-12">
          {filteredProducts.length > 0 ? (
            ...
            <div className="text-center py-20 bg-gray-50 rounded-xl border-2 border-dashed border-gray-200">
              ...
              <Link href="/kids" className="text-black font-bold border-b-2 border-black hover:pb-1 transition-all">
                Browse All Kids' Fashion
              </Link>
            </div>
          )}
        </div>
